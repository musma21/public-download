...
(() => {
  ...
  let gameState = {
    ...
    messagesQueue: [],
    freezeTime: 0 // 일시 freeze (레벨 완료 등)
  };

  // Sound Manager (간단, mute 지원)
  const SFX = {
  ...
  function createBall() {
    const angleDeg = 45 + Math.random() * 90; // 45~135도 사이
  ...
  function update(dt) {
    ...
    if (ball.sticky) {
      ...
      if (ball.y + ball.r >= paddle.y &&
        ...
        const maxBounceAngle = 75 * Math.PI / 180; // 75도
    ...
    for (const b of gameState.bricks) {
      ...
      if (dx * dx + dy * dy <= ball.r * ball.r) {
        // 충돌
        // 반사: 충돌 이전 위치로 되돌리는 간단 방식
        // 더 정확하게 하려면 이전 프레임 위치 저장 필요
  ...
  function draw() {
    ...
    drawPowerups();
    drawHUDInline();
    drawMessages(0); // dt는 메시지 렌더링에서 투명도 처리 때만 필요
  ...
  function drawBricks() {
    for (const b of gameState.bricks) {
      ...
      const hue = 40 + 200 * hpRatio; // 색상 변환
  ...
  function loop(now) {
    ...
    if (!gameState.paused) {
      update(dt);
      draw();
      // 메시지 투명도 갱신(위 drawMessages에서 dt 사용하도록 분리)
      updateMessages(dt);
    }
    requestAnimationFrame(loop);
  }
  ...
  // Rounded rect polyfill for Safari if needed
